    ...
    Register = async (req, res) => {
       try {
        ...
        if(!containsSpecialCharacter){
          ...
          return  sendResponse(res,406,"Invalid Domain Name",{});
        }
          const user = await User.findOne({ domain: req.body.domain })
          if (user) {
            ...
            const token=jwt.sign({id:user._id,domain:user.domain,appid:user.appid},process.env.JWT_SECRET,{expiresIn:"1d"})
...
Login = async (req, res) => {
      try {
        if(!req.body.domain){
          return  sendResponse(res,400,"please enter domain name",{});
        }
        else if(!req.body.password){
          return  sendResponse(res,400,"please enter password",{});
        }
        const isValidHostname = await checkDomainAvailability(req.body.domain);
          if(!isValidHostname){
          return  sendResponse(res,406,"Invalid Domain Name",{});
          ...
          if (user) {
            ...
            if(decrypted==req.body.password){
              ...
              return sendResponse(res,406,"please enter valid password")
            }
          }
          else if (!user) {
