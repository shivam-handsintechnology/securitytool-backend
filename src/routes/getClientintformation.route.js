...
router.post("/createuserdetails", async (req, res) => {
  ...
  if (alloweddomains) {
    const finduser = await Project_Security_Logs.findOne({ ip });
    if (finduser) {
      await Project_Security_Logs.findOneAndUpdate({ ip }, update);
    } else if (!finduser) {
...
router.get("/passwordkeys", async (req, res) => {
  const data = await PasswordKeyModel.find(
    {},
    { passwordkey: 1, _id: 0, exist: true }
  );
    ...
    if (alloweddomains) {
      ...
      if (sensitivedatainbody) {
        const existingMessage = await sensitiveinfoinbodyModel.findOne(
          { hostname, sensitivekeys: sensitivedatainbody },
          { _id: 0, exist: true }
        );
        if (existingMessage) {
...
router.post("/logsdata", async (req, res) => {
  ...
  if (alloweddomains) {
    const finduser = await ClientLoagsModel.findOne({ hostname: sid });
    if (finduser) {
