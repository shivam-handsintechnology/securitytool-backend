...
async function checkAutoComplete(url) {
  ...
  console.log("Please Wait Scanning..." + url)
  ...
  try {
    ...
    if (forms.length > 0) {
      for (const form of forms) {
        ...
        if (inputFields.length > 0) {
          ...
          if (sensitiveFields.length > 0) {
             ...
             sensitiveFields.forEach(field => console.log(`Auto-complete is enabled for the  sensitive fields ${field} on the Page ${pageName}`))

          } else {
            console.log('Auto-complete is disabled for all sensitive fields in :' + pageName);
          }
        } else {
          console.log('No input fields found i page'+pageName);
        }
      }
      ...
      if (captchaElements.length > 0) {
        console.log('CAPTCHA is implemented for publicly available forms on page.'+pageName);
      } 
    }
    ...
    if (iframes.length > 0) {
      console.log();
      iframes.forEach(iframe => console.log(`The page ${pageName} contains iframes which may be indicative of clickjacking vulnerability.`));
    }
    // return {
    ...
    if (error.name === 'TimeoutError') {
      console.log(`The page  ${pageName}  took too long to load.`);
    } else {
...
const test=async (websiteUrl) => {
  try {
    const visitedLinks = await scrapWebsite(websiteUrl);
    let links = Array.from(visitedLinks);
    links.forEach(async link => {
      await checkAutoComplete(link).catch(error => console.error(error));
    }
    )
